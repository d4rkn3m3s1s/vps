  ...
  return (
    <PageMotion className="page">
      ...
      <div className="section-grid groups-grid">
        ...
        <div className="panel">
          <h2>
            <Boxes size={16} style={{ marginRight: 6, verticalAlign: 'middle' }} /> Gruplar
          </h2>
                    ...
                    {editingId === g.id ? (
                      ...
                      <>
                        <button type="button" className="group-item-main" onClick={() => setSelected(g.id)}>
                          ...
                          <span className="group-count">{count} cihaz</span>
                        </button>
        ...
        <div className="panel">
          {!activeGroup ? (
            ...
            <>
              ...
              {/* Devices in group */}
              <h3 className="adb-subhead">Bu grupta ({inGroup.length})</h3>
              {inGroup.length === 0 ? (
                ...
                <div className="group-device-list">
                  {inGroup.map((d) => (
                    <div key={d.id} className="group-device-row">
                      ...
                      <button type="button" className="btn-ghost group-move-btn" disabled={busy} onClick={() => moveDevice(d.id, null)}>Çıkar</button>
                    </div>
                  ))}
                </div>
              )}
              ...
              {ungrouped.length > 0 ? (
                <>
                  <h3 className="adb-subhead">Cihaz ekle ({ungrouped.length} grupsuz)</h3>
                  <div className="group-device-list">
                    {ungrouped.map((d) => (
                      <div key={d.id} className="group-device-row">
                        <span className="group-device-name">{d.name}</span>
                        <button type="button" className="btn-primary group-move-btn" disabled={busy} onClick={() => moveDevice(d.id, activeGroup.id)}>Ekle</button>
                      </div>
                    ))}
                  </div>
                </>
